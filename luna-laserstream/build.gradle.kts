...
publishing {
    publications {
        create<MavenPublication>("mavenJava") {
            ...
            artifactId = "luna-laserstream"
            version = project.version.toString()

            pom {
                name.set("LunaSDK LaserStream")
                description.set(
                    "Helius LaserStream + Enhanced WebSocket subscriptions for LunaSDK — " +
                        "Flow-based account/transaction streams, geo-affinity endpoint selection, " +
                        "exponential-backoff reconnect, and a BYO gRPC transport interface."
                )
