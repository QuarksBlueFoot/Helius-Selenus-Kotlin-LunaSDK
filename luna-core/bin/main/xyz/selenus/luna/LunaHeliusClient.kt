...
class LunaHeliusClient(
    /**
     * Helius API key. Public so feature modules (`luna-privacy.RpcRotationApi`,
     * `luna-innovations.WebSocketEnhancedApi`/`LaserStreamAdvancedApi`) can build
     * their own WebSocket / gRPC URLs that embed `?api-key=$apiKey`. Treat as
     * a secret — do not log.
     */
    val apiKey: String,
    val cluster: Cluster = Cluster.MAINNET,
    /**
     * Shared [OkHttpClient]. Public so feature modules (luna-jupiter, third-party
     * REST integrations, etc.) reuse the same connection pool, interceptors and
     * timeouts as the core `rpcCall` / `restCall` transport.
     */
    val httpClient: OkHttpClient = OkHttpClient(),
    /**
     * Shared [Json] codec. Public for the same reason as [httpClient] —
     * feature modules decoding Helius / third-party responses should reuse
     * the same `ignoreUnknownKeys = true; encodeDefaults = true` configuration.
     */
     ...
     *
     * Public so feature modules (`luna-privacy.RpcRotationApi`) can include the
     * Helius endpoint in rotation lists without duplicating the cluster mapping.
     */
    val baseUrl: String
        get() = when (cluster) {
     ...
     */
    /**
     * Execute a REST API call against `https://api.helius.xyz/v0/<endpoint>`.
     *
     * Public because feature modules (`luna-enhanced-tx`, etc.) need to reach
     * the Enhanced Transactions REST surface that isn't exposed via JSON-RPC.
     * Prefer one of the typed namespace APIs when available.
     */
    @Throws(Exception::class)
    ...
    /**
     * Fetches the 75th percentile tip floor from Jito's public tip_floor endpoint.
     * Returns null if the fetch fails. Public so feature modules that build
     * tip-aware transactions (`luna-enhanced-tx`, Jupiter MEV helpers) can use it.
     */
    suspend fun fetchTipFloor(): Double? {
        val request = Request.Builder()
    ...
    /**
     * Send a transaction via the Helius Sender ultra-low-latency endpoint,
     * optionally restricted to SWQOS routing. Public so feature modules that
     * front-run tip-adjusted sends can reuse the same transport.
     */
    suspend fun sendViaSender(
        transaction: String,
        region: SenderRegion,
        swqosOnly: Boolean
    ): String {
    ...
    /**
     * Pre-warm a Helius Sender connection by hitting `/ping` against [region].
     *
     * Returns the wall-clock RTT in milliseconds, or `null` if the ping
     * failed. Useful to:
     *  - Open a TCP/TLS session before a latency-critical send (saves the
     *    handshake cost on the first real `/fast` call).
     *  - Confirm the region is reachable before queuing a tip-bearing tx.
     *
     * Mirrors the Helius Rust SDK's `warm_sender_connection`.
     */
    suspend fun warmSenderConnection(region: SenderRegion = SenderRegion.DEFAULT): Long? {
        val request = Request.Builder()
            .url("${region.url}/ping")
            .get()
            .build()
    ...
    /**
     * Determine the tip amount in lamports for a Sender submission, defaulting
     * to the 75th-percentile floor with a hard floor of [minLamports].
     *
     * Logic: fetch [fetchTipFloor] (returns SOL), convert to lamports, and
     * clamp to at least [minLamports]. If the network probe fails, return
     * [minLamports] so the send still has a chance of landing.
     *
     * @param minLamports Hard floor — Helius Sender's documented minimum is
     *   1,000,000 lamports (0.001 SOL); never tip below this.
     *
     * Mirrors the Helius Rust SDK's `determine_tip_lamports`.
     */
    suspend fun determineTipLamports(minLamports: Long = 1_000_000L): Long {
        val tipFloorSol = fetchTipFloor() ?: return minLamports
        val lamports = (tipFloorSol * 1_000_000_000.0).toLong()
        return maxOf(lamports, minLamports)
    }
    ...
    @Suppress("ClassName")
    private val _das: _DasMigrationShim = _DasMigrationShim()
    @Suppress("ClassName")
    private val _priority: _PriorityMigrationShim = _PriorityMigrationShim()
    @Suppress("ClassName")
    private val _enhanced: _EnhancedMigrationShim = _EnhancedMigrationShim()
    @Suppress("ClassName")
    private val _jupiter: _JupiterMigrationShim = _JupiterMigrationShim()
    @Suppress("ClassName")
    private val _analytics: _AnalyticsMigrationShim = _AnalyticsMigrationShim()
    @Suppress("ClassName")
    private val _rpc: _RpcMigrationShim = _RpcMigrationShim()
    @Suppress("ClassName")
    private inner class _DasMigrationShim {
    /**
    ...
    @Suppress("ClassName")
    private inner class _PriorityMigrationShim {
    /**
    ...
    @Suppress("ClassName")
    private inner class _EnhancedMigrationShim {
    /**
    ...
    @Suppress("ClassName")
    private inner class _JupiterMigrationShim {
    /**
    ...
    @Suppress("ClassName")
    private inner class _AnalyticsMigrationShim {
    /**
        ...
        suspend fun getTokenHealthScore(mint: String): RpcResponse<LunaHeliusClient.TokenHealthScore> {
            ...
            val assetResponse = _das.getAsset(mint)
    ...
    /**
        ...
        suspend fun getWalletRiskScore(address: String): RpcResponse<LunaHeliusClient.WalletRiskScore> {
            ...
            val assets = _das.getAssetsByOwner(address, limit = 100)
    ...
    @Suppress("ClassName")
    private inner class _RpcMigrationShim {
    /**
    ...
    inner class TransactionApi {
        ...
        suspend fun getSmartTransactionPlan(transaction: String): RpcResponse<SmartTransactionPlan> {
            ...
            val feeResponse = _priority.getPriorityFeeEstimate(
    ...
    inner class MemoApi {
         ...
         * Uses `_enhanced.parseTransaction` to get the parsed transaction and extracts memos.
         */
        suspend fun getMemosForTransaction(signature: String): RpcResponse<List<String>> {
            val response = _enhanced.getTransactions(listOf(signature))
    ...
    inner class NicheApi {
         ...
         * Combines `solana.getBalance` and `_das.getAssetsByOwner`.
         */
        suspend fun getWalletPortfolio(address: String, limit: Int = 1000): RpcResponse<WalletPortfolio> {
            ...
            val assetsResponse = _das.getAssetsByOwner(address, limit = limit)
        ...
        suspend fun getTokenDeepDive(mint: String): RpcResponse<TokenDeepDive> {
            ...
            val metadata = _das.getAsset(mint)
        ...
        suspend fun verifyGameAccess(
            ...
            val assetsResponse = _das.getAssetsByOwner(address, limit = 1000)
        ...
        suspend fun getAllAssetsByOwner(ownerAddress: String, maxPages: Int = 50): RpcResponse<List<JsonElement>> {
            ...
            while (page <= maxPages) {
                val response = _das.getAssetsByOwner(ownerAddress, page = page, limit = 1000)
        ...
        suspend fun getAllAssetsByGroup(groupKey: String, groupValue: String, maxPages: Int = 50): RpcResponse<List<JsonElement>> {
            ...
            while (page <= maxPages) {
                val response = _das.getAssetsByGroup(groupKey, groupValue, page = page, limit = 1000)
                if (response.error != null) {
                    if (page == 1) return RpcResponse(error = response.error)
                    break 
                }
    ...
    inner class SnsApi {
        ...
        suspend fun getDomains(owner: String): RpcResponse<List<JsonElement>> {
            val response = _das.getAssetsByOwner(owner, limit = 1000)
            if (response.error != null) return RpcResponse(error = response.error)
    ...
    inner class MobileApi {
        ...
        suspend fun getAssetLite(assetId: String): RpcResponse<JsonObject> {
            val response = _das.getAsset(assetId)
            if (response.error != null) return RpcResponse(error = response.error)
    ...
    inner class PrivacyApi {
        ...
        suspend fun analyzeWalletPrivacy(address: String): RpcResponse<PrivacyScore> {
            ...
            val assetsResponse = _das.getAssetsByOwner(address, limit = 100)
            val items = assetsResponse.result?.jsonObject?.get("items")?.jsonArray
    ...
    inner class TransactionHistoryApi {
        ...
        inner class TransactionHistoryBuilder(private val address: String) {
            ...
            suspend fun execute(): RpcResponse<TransactionHistoryResult> {
                ...
                val response = _rpc.getTransactionsForAddress(
                    address = address,
    ...
    inner class TokenLaunchApi {
        ...
        suspend fun getEarlyHolders(mintAddress: String, limit: Int = 20): RpcResponse<JsonElement> {
            ...
            return _das.getTokenAccounts(mint = mintAddress, limit = limit)
        }
        ...
        suspend fun getHolderDistribution(mintAddress: String): RpcResponse<JsonElement> {
            val healthScore = _analytics.getTokenHealthScore(mintAddress)
    ...
    inner class WalletCorrelationApi {
        ...
        suspend fun findRelatedWallets(address: String, depth: Int = 2): RpcResponse<WalletCluster> {
            ...
            val assetsResponse = _das.getAssetsByOwner(address, limit = 50)
    ...
    inner class BatchOperationsApi {
        ...
        suspend fun getAssetsForMultiple(addresses: List<String>, limitPerAddress: Int = 100): RpcResponse<Map<String, List<JsonElement>>> {
            ...
            for (address in addresses) {
                val response = _das.getAssetsByOwner(address, limit = limitPerAddress)
        ...
        suspend fun analyzeMultipleWallets(addresses: List<String>): RpcResponse<Map<String, WalletRiskScore>> {
            ...
            for (address in addresses) {
                val riskResponse = _analytics.getWalletRiskScore(address)
    ...
    inner class StrategyEngineApi {
        ...
        suspend fun detectArbitrage(
            ...
            // Get quote: SOL -> Token
            val buyQuote = _jupiter.getQuote(solMint, tokenMint, baseAmount)
            ...
            // Get quote: Token -> SOL
            val sellQuote = _jupiter.getQuote(tokenMint, solMint, tokensReceived)
        ...
        suspend fun generateMomentumSignal(tokenMint: String): RpcResponse<StrategySignal> {
            // Get current price
            val currentPrice = _jupiter.getPrices(listOf(tokenMint))
    ...
    inner class ReactiveStreamApi {
        ...
        fun tokenAccountChanges(
            ...
            while (true) {
                val response = _das.getTokenAccounts(owner = owner)
        ...
        fun priorityFeeStream(
            ...
            while (true) {
                val response = _priority.getPriorityFeeEstimate()
    ...
    inner class ZkPrivacyApi {
        ...
        suspend fun analyzeCompressionPrivacy(address: String): RpcResponse<CompressionPrivacyAnalysis> {
            ...
            val regularAccounts = _das.getAssetsByOwner(address)
    ...
    inner class ConfidentialTransactionApi {
        ...
        suspend fun calculatePrivacyOptimalParams(
            ...
            val priorityFee = _priority.getPriorityFeeEstimate()
        ...
        suspend fun analyzeRecipientPrivacyRisk(recipientAddress: String): RpcResponse<RecipientPrivacyRisk> {
            ...
            val txHistory = _rpc.getTransactionsForAddress(
                ...
                for (alert in alerts.filter { it.isActive }) {
                    when (alert.type) {
                        ...
                        "TRANSACTION_RECEIVED" -> {
                            val txResponse = _rpc.getTransactionsForAddress(alert.walletAddress, limit = 1)
                        ...
                        "LARGE_TRANSFER" -> {
                            val txResponse = _rpc.getTransactionsForAddress(alert.walletAddress, limit = 5)
    ...
    inner class MobileOptimizationApi {
        ...
        suspend fun getCompactWalletSummary(walletAddress: String): CompactWalletSummary {
            ...
            val tokens = _das.getTokenAccounts(owner = walletAddress)
    ...
    inner class StealthAddressApi {
        ...
        suspend fun analyzeStealthCharacteristics(address: String): RpcResponse<StealthAnalysis> {
            ...
            val txHistory = _rpc.getTransactionsForAddress(address, limit = 100)
            val transactions = txHistory.result?.jsonObject?.get("data")?.jsonArray
    ...
    inner class PrivacyPoolApi {
        ...
        suspend fun analyzePrivacyPoolParticipation(owner: String): RpcResponse<PrivacyPoolParticipation> {
            ...
            val regularAssets = _das.getAssetsByOwner(owner)
    ...
    inner class TransactionGraphPrivacyApi {
        ...
        suspend fun detectWalletLinkage(
            ...
            val tx1 = _rpc.getTransactionsForAddress(wallet1, limit = 50)
            val tx2 = _rpc.getTransactionsForAddress(wallet2, limit = 50)
    ...
    inner class ShieldedPatternApi {
        ...
        suspend fun analyzeTokenPrivacy(owner: String): RpcResponse<TokenPrivacyAnalysis> {
            ...
            // Get regular tokens
            val regularTokens = _das.getTokenAccounts(owner = owner)
    ...
    inner class PrivacyCombinatorApi {
        ...
        suspend fun executePrivacySwap(
            ...
            val swapTx = _jupiter.getSwapTransaction(
        ...
        suspend fun detectSurveillance(address: String): RpcResponse<SurveillanceAnalysis> {
            ...
            for (tx in recentTxs.take(20)) {
                ...
                val parsed = _enhanced.getTransactions(listOf(sig))
            ...
            val assets = _das.getAssetsByOwner(address, limit = 10)
    ...
    inner class AdvancedStealthApi {
        ...
        private fun generateSecureEntropy(size: Int): ByteArray {
            ...
            // derive future ephemeral hints. Fixed in v5.7.1 to use
            // xyz.selenus.luna.crypto.SecureRng (shared SecureRandom).
            return xyz.selenus.luna.crypto.SecureRng.nextBytes(size)
        }
        ...
        suspend fun scanForStealthPayments(
            ...
            for (sig in sigs.take(50)) {
                ...
                val txDetails = _enhanced.getTransactions(listOf(signature))
    ...
    inner class PrivateTransactionsApi {
        ...
        suspend fun analyzeTransactionPrivacy(signature: String): RpcResponse<TransactionPrivacyAnalysis> {
            val txResult = _enhanced.getTransactions(listOf(signature))
